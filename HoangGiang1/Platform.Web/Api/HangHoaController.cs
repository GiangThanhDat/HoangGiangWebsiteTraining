...
namespace Platform.Web.Api
{
    ...
    public class HangHoaController : ApiControllerBase
    {
        ...
        [HttpGet]
        public HttpResponseMessage getHangHoaForThietLap(HttpRequestMessage request)
        {
            return CreateHttpResponse(request, () =>
            {
                ...
                if (hangHoa != null)
                {
                    foreach (var item in hangHoa)
                    {
                        var tinhchat = _tinhChatHangHoaService.GetByID(item.MaTinhChat);
                        item.TinhChatHangHoa = Mapper.Map<TinhChatHangHoa, TinhChatHangHoaViewModel>(tinhchat);
