(function (app) {
    ...
    app.controller('DonDatHangController', DonDatHangController);

    DonDatHangController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function DonDatHangController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/dondathang/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaDonDatHang.split('ĐH')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaDonDatHang = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaDonDatHang: $scope.PhieuThu.MaDonDatHang, SoLuong: 0,GiaTriNhapKho:0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao,tenhang,dvt) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaDonDatHang: $scope.PhieuThu.MaDonDatHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat,ChietKhau:ck}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/dondathang/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    apiService.post('/api/chitietdondathang/create', $scope.ChiTietThu, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck,tenhang,dvt ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaDonDatHang: $scope.PhieuThu.MaDonDatHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0,TenHang:tenhang,DonViTinh:dvt,GiaKhuyenMai:km,VAT:vat,ChietKhau:ck}
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                $scope.ChiTietThu[a] = { MaHang: mahang, MaDonDatHang: $scope.PhieuThu.MaDonDatHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat,}
                ...
                for (var i = 0; i < $scope.AllChiTietBaoGia.length; i++) {
                    ...
                    angular.forEach($scope.AllHangHoa, function (item) {
                        if (item.MaHang == $scope.AllChiTietBaoGia[i].MaHang) {
                            ...
                            $scope.ChiTietThu[i].MaDonDatHang = $scope.PhieuThu.MaDonDatHang;
