(function (app) {
    ...
    app.controller('ChungTuBanHangController', ChungTuBanHangController);

    ChungTuBanHangController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function ChungTuBanHangController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        $scope.PhieuThu.DienGiai = 'Bán hàng ' ;
        $scope.PhieuThu.LyDoXuat = 'Xuất kho bán hàng ' ;
        $scope.getKhachHang = function (item, item2) {
            $scope.timkiem = false;
            $scope.option = true;
            $scope.PhieuThu.MaKhachHang = item;
            $scope.PhieuThu.DienGiai = 'Bán hàng ' + item2;
            $scope.PhieuThu.LyDoXuat = 'Xuất kho bán hàng ' + item2;
        }
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao, tenhang, dvt, TKCongNo_ChiPhi, TKDoanhThu) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaChungTuBanHang: $scope.PhieuThu.MaChungTuBanHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, TKCongNo_ChiPhi: TKCongNo_ChiPhi, TKDoanhThu: TKDoanhThu}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/chungtubanhang/create', $scope.PhieuThu, function (result) {
                    ...
                    // Lưu phiếu xuất
                    if ($scope.Kiemphieuxuatkho==true) {
                        apiService.post('/api/phieuxuat_banhang/create', $scope.PhieuThu, function (result) {
                            ...
                            apiService.post('/api/chitietphieuxuat_banhang/create', $scope.ChiTietThu, function (result) {
                                ...
                                if ($scope.Kiemphieuxuatkho == true && $scope.Lapkemhoadon == false) {
                                    $scope.ChiTietThu = [];
                                }
                            }, function () {
                    ...
                    if ($scope.Lapkemhoadon == true) {
                        apiService.post('/api/hoadon_banhang/create', $scope.PhieuThu, function (result) {
                            ...
                            apiService.post('/api/chitiethoadon_banhang/create', $scope.ChiTietThu, function (result) {
                                notificationService.displaySuccess("Lưu chi tiết hóa đơn thành công");
                                if (($scope.Kiemphieuxuatkho == false && $scope.Lapkemhoadon == true) || ($scope.Kiemphieuxuatkho == true && $scope.Lapkemhoadon == true)) {
                                    $scope.ChiTietThu = [];
                                }

                            }, function () {
                                    notificationService.displayWarning("Lưu chi tiết hóa đơn không thành công");
                    ...
                    if ($scope.Kiemphieuxuatkho == false && $scope.Lapkemhoadon == false) {
                        $scope.ChiTietThu = [];
                    }
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck,tenhang,dvt ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[a] = {
                    MaHang: mahang, MaChungTuBanHang: $scope.PhieuThu.MaChungTuBanHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat,ChietKhau:ck, TKCongNo_ChiPhi :TKCongNo_ChiPhi,TKDoanhThu :TKDoanhThu}
        ...
        function getChiTietBaoGia(id) {
            ...
            apiService.get('/api/chitietbaogia/getbyid', config, function (result) {
        ...
        $scope.newPhieuThu = function (selected) {
            if (selected) {
                ...
                $scope.PhieuThu.MaKhachHang = selected.originalObject.MaKhachHang;
                $scope.PhieuThu.MaSoBaoGia = selected.originalObject.MaSoBaoGia;
                ...
                $scope.getChiTietBaoGia(selected.originalObject.MaSoBaoGia);
