(function (app) {
    ...
    app.controller('ThongkeTraLaiHangBanController', ThongkeTraLaiHangBanController);

    ThongkeTraLaiHangBanController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService', '$http'];
    function ThongkeTraLaiHangBanController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        $scope.gridOptions = {
        ...
        $scope.getfromtoday = function (ngaydau, ngaycuoi) {
            ...
            apiService.get('/api/tralaihangban/gettralaihangban', config, function (result) {

                $scope.gettralaihangban = result.data;
        ...
        $scope.gridOptions.columnDefs = [
            { name: 'xemchitiet', width: 200, cellTemplate: '<span  ng-click="grid.appScope.xemTLbanhang(row)">Xem chi tiết</span>', headerCellClass: 'text-center' },
            ...
            { name: 'MaTraLaiHangBan', width: 200, pinnedRight: false, headerCellClass: 'text-center' },
        ...
        $scope.xemTLbanhang = function (row) {
            var index = row.entity.MaTraLaiHangBan;
            $scope.singer = {}
            $('.bs-example-modal-lg').modal('show');
            angular.forEach($scope.Data, function (i) {

                if (i.MaTraLaiHangBan == index) {
                    $scope.singer = i;
                }
            ...
            var config = {
                params: {
                    MaTraLaiHangBan: index
                }
            }
            apiService.get('/api/chitiettralaihangban/getchitiettralaihangban', config, function (result) {
