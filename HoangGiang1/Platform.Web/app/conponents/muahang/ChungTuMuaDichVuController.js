(function (app) {
    ...
    app.controller('ChungTuMuaDichVuController', ChungTuMuaDichVuController);

    ChungTuMuaDichVuController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function ChungTuMuaDichVuController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function GetAllHangHoa() {
            ...
            apiService.get('/api/dichvu/getall', null, function (result) {
        ...
        function getPhieuThu() {
            apiService.get('/api/chungtumuadichvu/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaChungTuMuaDichVu.split('MDV')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaChungTuMuaDichVu = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaChungTuMuaDichVu: $scope.PhieuThu.MaChungTuMuaDichVu, SoLuong: 0,GiaTriNhapKho:0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, TKChiPhi_TKKho, TKCongNo, DoiTuong, TenDoiTuong) {
            ...
            $scope.ChiTietThu[i] = { MaDichVu: mh, MaChungTuMuaDichVu: $scope.PhieuThu.MaChungTuMuaDichVu, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TKChiPhi_TKKho: TKChiPhi_TKKho, TKCongNo: TKCongNo, DoiTuong: DoiTuong, TenDoiTuong: TenDoiTuong}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $scope.PhieuThu.TienDichVu = total;
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/chungtumuadichvu/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    apiService.post('/api/chitietchungtumuadichvu/create', $scope.ChiTietThu, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaDichVu: mahang, MaChungTuMuaDichVu: $scope.PhieuThu.MaChungTuMuaDichVu, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0 }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                angular.forEach($scope.ChiTietThu, function (item) {
                    if (item.MaDichVu == mahang) {
                        TKChiPhi_TKKho = item.TKChiPhi_TKKho;
                ...
                $scope.ChiTietThu[a] = { MaDichVu: mahang, MaChungTuMuaDichVu: $scope.PhieuThu.MaChungTuMuaDichVu, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TKChiPhi_TKKho: TKChiPhi_TKKho, TKCongNo: TKCongNo, DoiTuong: DoiTuong, TenDoiTuong: TenDoiTuong}
