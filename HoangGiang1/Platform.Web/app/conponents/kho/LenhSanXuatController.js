(function (app) {
    ...
    app.controller('LenhSanXuatController', LenhSanXuatController);

    LenhSanXuatController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function LenhSanXuatController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/lenhsanxuat/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaLenhSanXuat;
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                //var pad = "NK"
                //var ans = pad.substring(0) + ans1
                $scope.PhieuThu.MaLenhSanXuat = ans1
                //alert(maso);
            }, function () {
                console.log('Tải thông tin thất bại');
            });
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaLenhSanXuat: $scope.PhieuThu.MaLenhSanXuat, SoLuong: 0, GiaTriNhapKho: 0, ThanhTien: 0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao, tenhang, dvt, Kho, TKNo, TKCo, ThanhPham, SoLo, HanSuDung, MaLenhSanXuat) {
            ...
            $scope.ChiTietThu[i] = { MaThanhPham: mh, MaLenhSanXuat: $scope.PhieuThu.MaLenhSanXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenThanhPham: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/lenhsanxuat/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    //Lưu chi tiết chứng từ
                    apiService.post('/api/lenhsanxuat_thanhpham/create', $scope.ChiTietThu, function (result) {
                   ...
                   apiService.post('/api/lenhsanxuat_nvl/create', $scope.ChiTietThu1, function (result) {
                            //notificationService.displaySuccess("Lưu chi tiết thành công");
                        $scope.ChiTietThu1 = [];
                    ...
                    $scope.PhieuThu = {
                        Ngay: new Date(), NgayChungTu: new Date(), NgayHoaDon: new Date()
                    }
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, tenhang, dvt, ThanhPham, SoLo, HanSuDung, DonDatHang, DoiTuongDHCP, MaThongKe,HopDongBan) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaThanhPham: mahang, MaLenhSanXuat: $scope.PhieuThu.MaLenhSanXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0, TenThanhPham: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, DonDatHang: DonDatHang, DoiTuongDHCP: DoiTuongDHCP, MaThongKe: MaThongKe, HopDongBan: HopDongBan }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                $scope.ChiTietThu[a] = {
                    MaThanhPham: mahang, MaLenhSanXuat: $scope.PhieuThu.MaLenhSanXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TenThanhPham: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat, DonDatHang: DonDatHang, DoiTuongDHCP: DoiTuongDHCP, MaThongKe: MaThongKe, HopDongBan: HopDongBan}
        ...
        $scope.themdong1 = function () {
            ...
            $scope.ChiTietThu1[$scope.clicks1 - 1] = { MaLenhSanXuat: $scope.PhieuThu.MaLenhSanXuat}
