       ...
       // [Authorize(Roles = "ViewUser")]
        ...
        public HttpResponseMessage GetUserByUsername(HttpRequestMessage request, string username)
        {
            ...
            else
            {
                ...
                var chucvu = _chucVuService.getChucVu(applicationUserViewModel.UserName);
                 applicationUserViewModel.ChucVu = Mapper.Map<IEnumerable<ChucVu>, IEnumerable<ChucVuViewModel>>(chucvu);
                return request.CreateResponse(HttpStatusCode.OK, applicationUserViewModel);
            }
        }
    }
