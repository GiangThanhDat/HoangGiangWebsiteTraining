(function (app) {
    ...
    app.controller('ChiTienController', ChiTienController);

    ChiTienController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function ChiTienController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function GetAllLoaiThu() {
            ...
            apiService.get('/api/loaichi/getall', null, function (result) {
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaPhieuChi: $scope.PhieuThu.MaPhieuChi, MaNhaCungCap: ma, TenNhaCungCap: ten }
        ...
        $scope.deletesolanlap = function (item) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = null
            $scope.clickss--;
        }
        $scope.ChiTietThu = [];
        $scope.ThanhTien = [];
        $scope.tinhThanhTien = function (tien, i) {
            $scope.ThanhTien[i] = { TongTienHang: parseInt(tien) }
        }
        $scope.TienThueGTGT = [];
        ...
        //    $scope.ChiTietThu[i] = { MaHang: mh, MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoLuong: sl, SoTien: TTTT }


        //}
        $scope.luutaikhoanvaochitietthu = function (ma, ten, i, diengiai, taikhoanno, taikhoanco, sotien, tknh, mucthichi, donvi, congtrinh, hopdongban, mathongke) {
            $scope.ChiTietThu[i] = { MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoTien: sotien, DienGiai: diengiai, SoTaiKhoanCo: taikhoanco, SoTaiKhoanNo: taikhoanno, MaNhaCungCap: ma, TenNhaCungCap: ten, TaiKhoanNganHang: tknh, MucThuChi: mucthichi, DonVi: donvi, CongTrinh: congtrinh, HopDongBan: hopdongban, MaThongKe: mathongke }
        }
        $scope.luutaikhoanno = function (ma, ten, i, diengiai, taikhoanno, taikhoanco, sotien, tknh, mucthichi, donvi, congtrinh, hopdongban, mathongke) {
            $scope.ChiTietThu[i] = { MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoTien: sotien, DienGiai: diengiai, SoTaiKhoanCo: taikhoanco, SoTaiKhoanNo: taikhoanno, MaNhaCungCap: ma, TenNhaCungCap: ten, TaiKhoanNganHang: tknh, MucThuChi: mucthichi, DonVi: donvi, CongTrinh: congtrinh, HopDongBan: hopdongban, MaThongKe: mathongke }
        }
        $scope.luutaikhoanco = function (ma, ten, i, diengiai, taikhoanno, taikhoanco, sotien, tknh, mucthichi, donvi, congtrinh, hopdongban, mathongke) {
            $scope.ChiTietThu[i] = { MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoTien: sotien, DienGiai: diengiai, SoTaiKhoanCo: taikhoanco, SoTaiKhoanNo: taikhoanno, MaNhaCungCap: ma, TenNhaCungCap: ten, TaiKhoanNganHang: tknh, MucThuChi: mucthichi, DonVi: donvi, CongTrinh: congtrinh, HopDongBan: hopdongban, MaThongKe: mathongke }
        }
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $scope.PhieuThu.TongTienThanhToan = total;
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {

                apiService.post('/api/phieuchi/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    angular.forEach($scope.ChiTietThu, function (item) {
                        if (item != null) {


                            apiService.post('/api/chitietphieuchi/create', item, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoLuong: sl, SoTien: TTTT }
            }
            else {
                ...
                $scope.ChiTietThu[a] = { MaHang: mahang, MaPhieuChi: $scope.PhieuThu.MaPhieuChi, SoLuong: sl, SoTien: TTTT }
