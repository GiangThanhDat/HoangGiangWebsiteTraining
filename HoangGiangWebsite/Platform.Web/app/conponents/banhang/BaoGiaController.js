(function (app) {
    ...
    app.controller('BaoGiaController', BaoGiaController);

    BaoGiaController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function BaoGiaController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/baogia/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaSoBaoGia.split('BG')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaSoBaoGia: $scope.PhieuThu.MaSoBaoGia, SoLuong: 0,GiaTriNhapKho:0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaSoBaoGia: $scope.PhieuThu.MaSoBaoGia, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $scope.PhieuThu.TienHang = total;
            $scope.PhieuThu.TienThueGTGT = total1;
            $scope.PhieuThu.TongTien = total2;
            $scope.PhieuThu.TienChietKhau = 0;
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {

                apiService.post('/api/baogia/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    apiService.post('/api/chitietbaogia/create', $scope.ChiTietThu, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaSoBaoGia: $scope.PhieuThu.MaSoBaoGia, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0 }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                //    }
                //})
                $scope.ChiTietThu[a] = { MaHang: mahang, MaSoBaoGia: $scope.PhieuThu.MaSoBaoGia, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien}
