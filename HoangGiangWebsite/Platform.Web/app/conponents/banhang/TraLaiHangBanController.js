(function (app) {
    ...
    app.controller('TraLaiHangBanController', TraLaiHangBanController);

    TraLaiHangBanController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService', '$http'];
    function TraLaiHangBanController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/tralaihangban/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaTraLaiHangBan.split('BTL')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaTraLaiHangBan = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaTraLaiHangBan: $scope.PhieuThu.MaTraLaiHangBan, SoLuong: 0, GiaTriNhapKho: 0 }
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao, tenhang, dvt, TKTraLai, TKTien,MucThuChi) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaTraLaiHangBan: $scope.PhieuThu.MaTraLaiHangBan, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, TKTraLai: TKTraLai, TKTien: TKTien, MucThuChi: MucThuChi }
        ...
        $scope.LuuPhieuThu = function () {
            ...
            angular.forEach($scope.ChiTietThu, function (item) {
                ...
                item.MaTraLaiHangBan = $scope.PhieuThu.MaTraLaiHangBan
            ...
            if ($scope.show == 'Giảm trừ công nợ') {
                link = 'tralaihangban'
                chitietlink = 'chitiettralaihangban'
            }
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/' + link + '/create', $scope.PhieuThu, function (result) {
                    ...
                    if ($scope.Kiemphieunhapkho == true) {
                        apiService.post('/api/phieuxuat_banhang/create', $scope.PhieuThu, function (result) {
                            notificationService.displaySuccess("Lưu phiếu xuất kho thành công");
                            getPhieuXuat();
                            //Lưu chi tiết phiếu xuất
                            apiService.post('/api/chitietphieuxuat_banhang/create', $scope.ChiTietThu, function (result) {
                                notificationService.displaySuccess("Lưu chi tiết phiếu xuất kho thành công");
                                if ($scope.Kiemphieunhapkho == true) {
                                    $scope.ChiTietThu = [];
                                }
                            }, function () {
                                notificationService.displayWarning("Lưu chi tiết phiếu xuất kho không thành công");

                            });


                        }, function () {
                            notificationService.displayWarning("Lưu phiếu xuất kho không thành công");
                    ...
                    if ($scope.Kiemphieunhapkho == false) {
                        $scope.ChiTietThu = [];
                    }
                    getPhieuChi();
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, tenhang, dvt) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaTraLaiHangBan: $scope.PhieuThu.MaTraLaiHangBan, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                angular.forEach($scope.ChiTietThu, function (item) {
                    if (item.MaHang == mahang) {
                        ...
                        TKTraLai = item.TKTraLai;
                        TKTien = item.TKTien;
                        MucThuChi = item.MucThuChi;

                    }
                })
                $scope.ChiTietThu[a] = { MaHang: mahang, MaTraLaiHangBan: $scope.PhieuThu.MaTraLaiHangBan, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, TKTraLai: TKTraLai, TKTien: TKTien, MucThuChi: MucThuChi }
                ...
                for (var i = 0; i < $scope.AllChiTietBaoGia.length; i++) {
                    ...
                    angular.forEach($scope.AllHangHoa, function (item) {
                        if (item.MaHang == $scope.AllChiTietBaoGia[i].MaHang) {
                            ...
                            $scope.ChiTietThu[i].MaTraLaiHangBan = $scope.PhieuThu.MaTraLaiHangBan;
        ...
        $scope.mophieuxuatkho = function () {
            $scope.chungtughino = false;
            $scope.phieuxuat = false;
            $scope.phieuchi = false;
            $scope.phieuxuatkho = true;
        }  
        ...
        function getPhieuXuat() {
            apiService.get('/api/phieuxuat_banhang/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuxuat = result.data[result.data.length - 1].MaPhieuXuat;
                }
                else {
                    $scope.maphieuxuat = 0;
                }
                ...
                var maso = parseInt($scope.maphieuxuat);
                ...
                //alert(ans);
                $scope.PhieuThu.MaPhieuXuat = ans1
