(function (app) {
    ...
    app.controller('ChungTuMuaHangNhieuHoaDonController', ChungTuMuaHangNhieuHoaDonController);

    ChungTuMuaHangNhieuHoaDonController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function ChungTuMuaHangNhieuHoaDonController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/chungtumuahang/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaChungTuMuaHang.split('NK')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaChungTuMuaHang = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaChungTuMuaHang: $scope.PhieuThu.MaChungTuMuaHang, SoLuong: 0,GiaTriNhapKho:0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, Kho, TKKho, TKCongNo, ChiPhiMuaHang, GiaTriNhapKho, SoLo, HanSuDung, DoiTuongDHCP, CongTrinh, MaDonMuaHang, MaChungTuMuaHang, MaThongKe) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaChungTuMuaHang: $scope.PhieuThu.MaChungTuMuaHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, Kho: Kho, TKKho: TKKho, TKCongNo: TKCongNo, ChiPhiMuaHang: ChiPhiMuaHang, GiaTriNhapKho: GiaTriNhapKho, SoLo: SoLo, HanSuDung: HanSuDung, DoiTuongDHCP: DoiTuongDHCP, CongTrinh: CongTrinh, MaDonMuaHang: MaDonMuaHang, MaThongKe: MaThongKe}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/chungtumuahang/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    apiService.post('/api/chitietchungtumuahang/create', $scope.ChiTietThu, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, ) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaChungTuMuaHang: $scope.PhieuThu.MaChungTuMuaHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0 }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                angular.forEach($scope.ChiTietThu, function (item) {
                    if (item.MaHang == mahang) {
                        ...
                        ChiPhiMuaHang = item.ChiPhiMuaHang;
                        GiaTriNhapKho = item.GiaTriNhapKho;
                        SoLo = item.SoLo;
                        HanSuDung = item.HanSuDung;
                        DoiTuongDHCP = item.DoiTuongDHCP;
                        CongTrinh = item.CongTrinh;
                        MaDonMuaHang = item.MaDonMuaHang;
                        MaThongKe = item.MaThongKe;
                    }
                })
                $scope.ChiTietThu[a] = { MaHang: mahang, MaChungTuMuaHang: $scope.PhieuThu.MaChungTuMuaHang, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien,  Kho: Kho, TKKho: TKKho, TKCongNo: TKCongNo, ChiPhiMuaHang: ChiPhiMuaHang, GiaTriNhapKho: GiaTriNhapKho, SoLo: SoLo, HanSuDung: HanSuDung, DoiTuongDHCP: DoiTuongDHCP, CongTrinh: CongTrinh, MaDonMuaHang: MaDonMuaHang, MaThongKe: MaThongKe}
