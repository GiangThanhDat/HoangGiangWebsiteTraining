(function (app) {
    ...
    app.controller('NhapKhoController', NhapKhoController);

    NhapKhoController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function NhapKhoController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/phieunhapkho/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaPhieuNhapKho.split('NK')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaPhieuNhapKho = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaPhieuNhapKho: $scope.PhieuThu.MaPhieuNhapKho, SoLuong: 0, GiaTriNhapKho: 0, ThanhTien: 0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao, tenhang, dvt, Kho, TKNo, TKCo, ThanhPham, SoLo, HanSuDung, MaLenhSanXuat) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaPhieuNhapKho: $scope.PhieuThu.MaPhieuNhapKho, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/phieunhapkho/create', $scope.PhieuThu, function (result) {
                    notificationService.displaySuccess("Lưu thành công");
                    //Lưu chi tiết chứng từ
                    apiService.post('/api/chitietphieunhapkho/create', $scope.ChiTietThu, function (result) {
        ...
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, tenhang, dvt, ThanhPham, SoLo, HanSuDung) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaPhieuNhapKho: $scope.PhieuThu.MaPhieuNhapKho, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung:HanSuDung }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                $scope.ChiTietThu[a] = {
                    MaHang: mahang, MaPhieuNhapKho: $scope.PhieuThu.MaPhieuNhapKho, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat}
        ...
        function getChiTietBaoGia(id) {
            ...
            apiService.get('/api/chitiettralaihangban/getbyid', config, function (result) {
                ...
                for (var i = 0; i < $scope.AllChiTietBaoGia.length; i++) {
                    ...
                    angular.forEach($scope.AllHangHoa, function (item) {
                        if (item.MaHang == $scope.AllChiTietBaoGia[i].MaHang) {
                            ...
                            $scope.ChiTietThu[i].MaPhieuNhapKho = $scope.PhieuThu.MaPhieuNhapKho;
        ...
        $scope.newPhieuThu = function (selected) {
            if (selected) {
                ...
                $scope.PhieuThu.MaKhachHang = selected.originalObject.MaKhachHang;
                $scope.PhieuThu.ChungTuThamChieu = selected.originalObject.MaTraLaiHangBan;
                ...
                $scope.getChiTietBaoGia(selected.originalObject.MaTraLaiHangBan);
