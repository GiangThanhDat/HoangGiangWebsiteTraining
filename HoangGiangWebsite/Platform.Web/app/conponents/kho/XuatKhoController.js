(function (app) {
    ...
    app.controller('XuatKhoController', XuatKhoController);

    XuatKhoController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService','$http'];
    function XuatKhoController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        ...
        function getPhieuThu() {
            apiService.get('/api/phieuxuatkho/getall', null, function (result) {
                if (result.data.length > 0) {
                    $scope.maphieuthu = result.data[result.data.length - 1].MaPhieuXuat.split('NK')[1];
                }
                else {
                    $scope.maphieuthu = 0;
                }
                ...
                var ans = pad.substring(0) + ans1
                //alert(ans);
                $scope.PhieuThu.MaPhieuXuat = ans
        ...
        $scope.themdong = function (ma, ten) {
            ...
            $scope.ChiTietThu[$scope.clickss - 1] = { MaPhieuXuat: $scope.PhieuThu.MaPhieuXuat, SoLuong: 0, GiaTriNhapKho: 0, ThanhTien: 0}
        ...
        $scope.tinhTienThueGTGT = function (km, sl, vat, i, ck, mh, SoLuongDaGiao, tenhang, dvt, Kho, TKNo, TKCo, ThanhPham, SoLo, HanSuDung, MaLenhSanXuat) {
            ...
            $scope.ChiTietThu[i] = { MaHang: mh, MaPhieuXuat: $scope.PhieuThu.MaPhieuXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, SoLuongDaGiao: SoLuongDaGiao, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat}
        ...
        $scope.LuuPhieuThu = function () {
            ...
            $ngBootbox.confirm('Bạn chắc muốn lưu  không').then(function () {
                ...
                apiService.post('/api/phieuxuat_banhang/create', $scope.PhieuThu, function (result) {
                    ...
                    $scope.KhacHang = {};
                    $scope.$broadcast('angucomplete-alt:clearInput'); getPhieuXuat(); getHoaDon();
        ...
        //#endregion
        $scope.LuuMaHang = [];
        $scope.checktrung = function (mahang, item, i, km, sl, vat, ck, tenhang, dvt, ThanhPham, SoLo, HanSuDung) {
            ...
            if (a == -1) {
                ...
                $scope.ChiTietThu[i] = { MaHang: mahang, MaPhieuXuat: $scope.PhieuThu.MaPhieuXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, GiaTriNhapKho: 0, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung:HanSuDung }
            }
            else {
                var index = $scope.solanlap.indexOf(item);
                ...
                $scope.ChiTietThu[a] = {
                    MaHang: mahang, MaPhieuXuat: $scope.PhieuThu.MaPhieuXuat, SoLuong: sl, ThanhTien: TTTT, DonGia: km, TienChietKhau: 0, TienThueGTGT: thanhtien, TenHang: tenhang, DonViTinh: dvt, GiaKhuyenMai: km, VAT: vat, ChietKhau: ck, Kho: Kho, TKNo: TKNo, TKCo: TKCo, ThanhPham: ThanhPham, SoLo: SoLo, HanSuDung: HanSuDung, MaLenhSanXuat: MaLenhSanXuat}
                ...
                for (var i = 0; i < $scope.AllChiTietBaoGia.length; i++) {
                    ...
                    angular.forEach($scope.AllHangHoa, function (item) {
                        if (item.MaHang == $scope.AllChiTietBaoGia[i].MaHang) {
                            ...
                            $scope.ChiTietThu[i].MaPhieuXuat = $scope.PhieuThu.MaPhieuXuat;
                            $scope.ChiTietThu[i].ThanhPham = item.ThanhPham;
                            $scope.ChiTietThu[i].SoLo = item.SoLo;
                            $scope.ChiTietThu[i].HanSuDung = item.HanSuDung;
        ...
        $scope.newPhieuThu = function (selected) {
            if (selected) {
                ...
                $scope.PhieuThu.MaKhachHang = selected.originalObject.MaKhachHang;
                $scope.PhieuThu.MaChungTuBanHang = selected.originalObject.MaChungTuBanHang;
                $scope.PhieuThu.DienGiai = selected.originalObject.DienGiai;
                console.log($scope.PhieuThu)
                angular.forEach($scope.AllKhachHang, function (item) {
                    if (item.MaKhachHang == selected.originalObject.MaKhachHang) {
                        ...
                        $scope.KhacHang.TenNguoiNhan = item.TenNguoiNhan;
                        $scope.PhieuThu.TenNguoiNhan = item.TenNguoiNhan;
                    }
                })
