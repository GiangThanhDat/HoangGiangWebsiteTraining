(function (app) {
    ...
    app.controller('ThongKeLenhSanXuatController', ThongKeLenhSanXuatController);

    ThongKeLenhSanXuatController.$inject = ['$scope', 'apiService', 'notificationService', '$ngBootbox', '$filter', 'commonService', '$http'];
    function ThongKeLenhSanXuatController($scope, apiService, notificationService, $ngBootbox, $filter, commonService, $http) {
        $scope.gridOptions = {
        ...
        $scope.getfromtoday = function (ngaydau, ngaycuoi) {
            ...
            apiService.get('/api/lenhsanxuat/getLenhSanXuat', config, function (result) {
                $scope.getLenhSanXuat = result.data;
                angular.forEach($scope.getLenhSanXuat, function (item) {
                ...
                $scope.Data = $scope.getLenhSanXuat;
        ...
        $scope.gridOptions.columnDefs = [
            { name: 'xemchitiet', width: 200, cellTemplate: '<span  ng-click="grid.appScope.xemLENHSX(row)">Xem chi tiết</span>', headerCellClass: 'text-center' },

            { name: 'Ngay', width: 200, enablePinning: false, headerCellClass: 'text-center' },
            { name: 'MaLenhSanXuat', width: 200, pinnedLeft: false, headerCellClass: 'text-center' },
            { name: 'DienGiai', width: 200, pinnedRight: false, headerCellClass: 'text-center'},
            { name: 'PN', width: 200, headerCellClass: 'text-center'},
            { name: 'PX', width: 200, headerCellClass: 'text-center' },
        ...
        $scope.gridOptions.columnDefs[3].displayName = 'Diễn giải'
        $scope.gridOptions.columnDefs[4].displayName = 'Đã lập đủ PN '
        $scope.gridOptions.columnDefs[5].displayName = 'TĐã lập đủ PX'
        $scope.gridOptions.columnDefs[6].displayName = 'Tình trạng '
        $scope.gridOptions.columnDefs[7].displayName = 'Chi nhánh'
      

        $scope.xemLENHSX = function (row) {
            var index = row.entity.MaLenhSanXuat;
         
                $('.bs-example-modal-lg').modal('show');
                angular.forEach($scope.Data, function (item) {

                    if (item.MaLenhSanXuat == index) {
                        ...
                        apiService.get('/api/lenhsanxuat_thanhpham/getchitietlenhsanxuatthanhpham', config, function (result) {

                            $scope.ChitietTP = result.data;
                        ...
                        apiService.get('/api/lenhsanxuat_nvl/getlenhsanxuatNVL', config, function (result) {

                            $scope.ChitietNVL = result.data;
